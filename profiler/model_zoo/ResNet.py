...
class BasicBlock(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        out = out + identity     # modified as non-inplace add
        out = self.relu(out)
...
class ResNet(nn.Module):
    def __init__(
        ...
        ###
        # replaced torch.flatten() in forward() with adding nn.Flatten() layer in model definition.
        # since module can be identified for profiling, while the function cannot. 
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        x = self.avgpool(x)
        # x = torch.flatten(x, 1)
        # original torch.flatten() restricts parameter types
        # such that our TensorWrapper could not fit.
        x = self.flatten(x)
        x = self.fc(x)
