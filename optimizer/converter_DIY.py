...
def convert_subgraph_to_module(graph, full_graph, num_subgraphs, module_name, initialize_weights,
    ...
    # copy each input to "output", then start operations.
    ...
    # concatenate the layer name with template.
    for node in nodes:
        layer_call = None
...
def start(profile_path, model_name, arch, output_directory):
    ...
    # import the computation graph from profile
    input_node = graph.Node("input_node", node_desc="Input")
    full_graph = graph.Graph.from_str(open(args.profile_filename, 'r').read())
    
    # some initializations. initialize weights for vgg16 and resnet50. (why?)
    # initialize stage id to 0.
    # obtain all sinks (node that not connected with any edges) in the computation graph.
    ...
    # (?) how sink node is generated in prevoius step?
    for sink in sinks:
        if sink.node_desc.startswith("__getitem__"):
            full_graph.remove_node(sink)
    subgraphs = full_graph.partition_graph()

    # Convert subgraphs to modules.
    # using pre-defined template to generate python code.
    # map the nodes & edges to torch modules and operations.
    # core function in convert_subgraph_to_module()
    ...
    # the result of the above code block: stage%d.py
    # each stage consists of divided layers
